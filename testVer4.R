...
test_that("sampling ok",{
  x <- rbeta(100,shape1=1.3,shape2=2.7)
  y <- ars(100,dbeta,shape1=1.3,shape2=2.7, x.start=c(0.001,0.999))
  wilcox_test <- wilcox.test(x, y, 
                             alternative = c("two.sided", "less", "greater"),
                             mu = 0, paired = FALSE, exact = NULL, correct = TRUE,
                             conf.int = FALSE, conf.level = 0.95)$p.value
...
## 4.1 rate=2
### Summary
mysample = ars(n = 100,dexp, rate=2, x.start=c(0.001,2),plot.type="bounds")
mysample = ars(100,dexp, rate=2, x.start=c(0.001,2),plot.type="acceptance")
...
test_that("sampling ok",{
  x <- rexp(100,rate=2)
  y <- ars(100,dexp, rate=2,x.start=c(0.001,2))
  wilcox_test <- wilcox.test(x, y, 
                             alternative = c("two.sided", "less", "greater"),
                             mu = 0, paired = FALSE, exact = NULL, correct = TRUE,
                             conf.int = FALSE, conf.level = 0.95)$p.value
...
test_that("sampling ok",{
  x <- rlogis(100, location=0, scale=1)
  y <- ars(100,dlogis,location=0, scale=1,x.start=c(-6,2))
  wilcox_test <- wilcox.test(x, y, 
                             alternative = c("two.sided", "less", "greater"),
                             mu = 0, paired = FALSE, exact = NULL, correct = TRUE,
                             conf.int = FALSE, conf.level = 0.95)$p.value
