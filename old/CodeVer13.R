...
ars = function(n, g, ..., dg=NULL, log.transform=FALSE, x.start=NULL, plot.type = "none"){ 
  ...
  if(length(x.start)!=0) {
    ...
    #if (g=="dnorm") {x.start = c(mean-2*sd,mean/2,mean+2*sd)} 
    ## Better if assign reasonable starting values for the known distributions 
    #else if (g=="dgamma") {x.start = c(,shape/scale,)}
    x.start = rep(NA,3)
    x.start[1] = 0
    while(exp(f(x.start[1]))<=0|exp(f(x.start[1]))==Inf) {
      x.start[1] = rnorm(1,mean=0,sd=10)
    }
    dx.start = 10 # Reasonable?
    x.start[2] = x.start[1] + dx.start
    x.start[3] = x.start[2] + dx.start
    while(min(exp(f(x.start)))<=0|max(exp(f(x.start)))==Inf) {
      dx.start = dx.start/2 
      x.start[2] = x.start[1] + dx.start
      x.start[3] = x.start[2] + dx.start
    }
...
y <- ars(n, dnorm, mean=0,sd=1, dg=NA, x.start=c(-10,1,10), plot.type="bounds")
...
mysample = ars(n = 100,dgamma, shape=2, scale=0.5, plot.type="bounds")
mysample = ars(n = 100,dgamma, shape=2, scale=0.5, plot.type="acceptance")
...
mysample = ars(n = 100, dbeta, shape1 = 1.3, shape2 = 2.7, plot.type="bounds")
mysample = ars(n = 100, dbeta, shape1 = 1.3, shape2 = 2.7, plot.type="acceptance")
...
## mysample = ars(n = 100, dexp, rate=2, plot.type="bounds")
## mysample = ars(100, dexp, rate=2, plot.type="acceptance")
