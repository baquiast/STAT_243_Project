...
### Summary
mysample = ars(100,dnorm,x.start=c(-2,2),plot.type="bounds")
mysample = ars(100,dnorm,x.start=c(-2,2),plot.type="acceptance")
...
test_that("sampling ok",{
  ...
  y <- ars(n,dnorm,mean=0,sd=1,x.start=c(-2,2))
...
## 5.1 df=2
### Summary - Right plot?
mysample = ars(100,dchisq,df=2,x.start=c(0.001,4),plot.type="bounds")
mysample = ars(100,dchisq,df=2,x.start=c(0.001,4),plot.type="acceptance")
...
test_that("sampling ok",{
  x <- rchisq(100,df=2)
  y <- ars(100,dchisq,df=2,x.start=c(0.001,4))
  wilcox_test <- wilcox.test(x, y, 
                             alternative = c("two.sided", "less", "greater"),
                             mu = 0, paired = FALSE, exact = NULL, correct = TRUE,
                             conf.int = FALSE, conf.level = 0.95)$p.value
...
### Summary
mysample = ars(n = 100,dlogis, location=0, scale=1, x.start=c(-2,2),plot.type="bounds")
mysample = ars(100,dlogis, location=0, scale=1, x.start=c(-2,2),plot.type="acceptance")
...
test_that("sampling ok",{
  x <- rlogis(100, location=0, scale=1)
  y <- ars(100,dlogis,location=0, scale=1,x.start=c(-2,2))
  wilcox_test <- wilcox.test(x, y, 
                             alternative = c("two.sided", "less", "greater"),
                             mu = 0, paired = FALSE, exact = NULL, correct = TRUE,
                             conf.int = FALSE, conf.level = 0.95)$p.value
...
test.sample = ars(100,g,x.start=c(-2,2))
test.sample = ars(100,g,x.start=c(-2,2),plot.type="bounds")
test.sample = ars(100,g,x.start=c(-2,2),plot.type="acceptance")
...
dg = function(x) {-x}
## Our function
system.time(ars(1000, g=g,dg=dg,log.transform=TRUE,x.start=c(-4,4)))
#  user  system elapsed 
# 0.116   0.004   0.144 
rm(list=ls())
g = function(x) {-0.5*x^2}
