...
rand.x <- runif(n=100,min=-100,max=100) # Random sample x's from unif distribution
...
x.out <- rep(NA,n) # If I want to sample out 100 x's
 # 1-1) Select the three starting points
start.select=0
...
lower.rand <- runif(n=1,min=l.max,max=0) # right?

if (x.cand < u.inter12) {u.max <- u.bound1(x.cand)
 } else if (x.cand < u.inter23) {u.max <- u.bound2(x.cand)
 } else {u.max <- u.bound3(x.cand)}
upper.rand <- runif(n=1,min=0,max=u.max) # right?

d.max <- f(x.cand)
dist.rand <- runif(n=1,min=d.max,max=0) # right?

 # 3) Updating step

x.sample <- x.out

if (u <= exp(lower.rand-upper.rand)) {
  x.out[4] <- x.cand
  } else if (u <= exp(dist.rand-upper.rand)) {
    x.out[4] <- x.cand
  }

 # If x is rejected at the squeezing step, use that x to make the upper and lower bounds
if (u > exp(lower.rand-upper.rand)) {
  x.sample[4] <- x.cand
